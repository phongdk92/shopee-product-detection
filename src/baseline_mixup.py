...
#                                   size=(224, 224),
#                                   valid_pct=0.2,
#                                   seed=42).normalize(imagenet_stats)
...
learn.fit_one_cycle(20, callbacks=[SaveModelCallback(learn, every='improvement', monitor='accuracy', name='best')])
learn.save('resnet152_bi.model', return_path=True)
il = ImageList.from_folder(test_path)
predicts = []
for image in tqdm(il):
    predicts.append(str(learn.predict(image)[0]))
...
submission[['filename', 'category']].to_csv('sub.csv', index=False)
